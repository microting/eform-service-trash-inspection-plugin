...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("ServiceTrashInspectionPlugin")
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.eFormTrashInspectionBase'
        ]
        process_repository('ServiceTrashInspectionPlugin.csproj', packages, 'eform-service-trashinspection-plugin')

        os.chdir("..")
        os.chdir("ServiceTrashInspectionPlugin.Integration.Test")
        packages = [
            'Microsoft.NET.Test.Sdk', 'NUnit', 'NUnit3TestAdapter', 'NUnit.Analyzers'
        ]
        process_repository('ServiceTrashInspectionPlugin.Integration.Test.csproj', packages, 'eform-service-trashinspection-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("..")
            run_command(f"github_changelog_generator -u microting -p eform-service-trashinspection-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
